...
#define MAT_READ_DONORS    1
#define MAT_READ_ACCEPTORS 2
#define MAT_READ_BOTH      3
#define MAT_RES_1          1
#define MAT_RES_2          2
...
void CalculateCaToCaVector(PDB *res1_start, PDB *res1_stop,
                           ...
                           VEC3F *CAtoCAVector);
BOOL FindAtom(PDB *start, PDB *stop, char *atnam, VEC3F *c_alpha);
BOOL CreateRotationMatrix(PDB *pdb, PDB *res1_start,
                          PDB *res1_stop, PDB *res2_start,
                          PDB *res2_stop, VEC3F CAtoCAVector, int atomset1, int atomset2);
...
/*      fprintf(stderr, "Unable to find one of the residues in the matrix file: %s %s\n", res1, res2); */
   ...
   */
   CreateRotationMatrix(pdb, res1_start, res1_stop, res2_start,
                        res2_stop, CAtoCAVector, ATOMS_NCACB, ATOMS_NCACB );
...
BOOL CreateRotationMatrix(PDB *pdb, PDB *res1_start, PDB *res1_stop,
 PDB *res2_start, PDB *res2_stop,VEC3F Vector, int atomset1, int atomset2)
{
   VEC3F tempv;   
...
BOOL FindAtom(PDB *res1_start, PDB *res1_next, char *atnam, VEC3F *atm)
{
   PDB *p;
   BOOL found = FALSE;
...
BOOL ParseCmdLine(int argc, char **argv, REAL *cutoff, BOOL *hbplus,
                  char *hatom1, char *hatom2,char *matrix_file, char *matrix_file2, char *pdbfile,
                  char *locres1, char *locres2, char *res2, 
                  char *outputfile)
{
   argc--;
...
/*************************************/
/* function that calculates the vector from the N of res1
 to CA of res2 */
void CalculateNToCaVector(PDB *res1_start, PDB *res1_stop,
                          PDB *res2_start, PDB *res2_stop, VEC3F *NtoCAVector)
{
                        ...
                        FILE *OUT)
{
   ...
   if(!ReadInMatrices(res1, res2, matrix, MAT_READ_ACCEPTORS, MAT_RES_2))
   {
      /* ACRM 08.09.05 - error message */
   ...
   if(!ReadInMatrices(res1, res2, matrix2, MAT_READ_DONORS, MAT_RES_1))
   {
      /* ACRM 08.09.05 - error message */
   ...
   CreateRotationMatrix(pdb, res1_start, res1_stop, res2_start,
                        res2_stop, NtoCAVector, ATOMS_CNCA, ATOMS_NCACB );
