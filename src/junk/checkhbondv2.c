...
void Usage(void);
BOOL ReadInMatrices(char *res1, char *res2, FILE *matrix, char *type);
...
BOOL ParseCmdLine(int argc, char **argv, REAL *cutoff,
                  BOOL *hbplus, char *type, char *hatom1,
...
/*BOOL IsHBondvoid */
BOOL PrepareHBondingPair(int resnum1, int resnum2, PDB *pdb, FILE *matrix, char *type, char *chain1, 
...
int main (int argc, char *argv[])
{
   ...
   char chain1[6], insert1[6], chain2[6], insert2[6], hatom1[6], hatom2[6], type[4];
   ...
   // A Cuff, December 2005.  Add flag on command line to indicate type of h bond to be assessed
   if(ParseCmdLine(argc, argv,  &cutoff, &hbplus, type,  hatom1, hatom2,
            ...
            {               
               ...
               { 
                  ...
                  if((pdb = ReadPDB(PDBFILE, &natoms)) !=NULL)
                  {
                     ...
                     if(!PrepareHBondingPair(resnum1, resnum2, pdb, matrix, type, chain1, chain2, 
                                             insert1, insert2, &hbplus, hatom1, hatom2,
                                             cutoff, res1, res2, OUT))
                     {
                        /* ACRM 08.09.05 Swap chain, inserts and hatom as well! */
                        if(!PrepareHBondingPair(resnum2, resnum1, pdb, matrix, type, chain2, chain1, 
                         ...
                         FILE *OUT)
{
   ...
   // A Cuff December 2005. Pass h bond type to be assessed to ReadInMatrices routine
   if(!ReadInMatrices(res1, res2, matrix, type))
   {
...
BOOL ReadInMatrices(char *res1, char *res2, FILE *matrix, char *type)
{
   ...
   {     
      ...
      // A Cuff December 2005. Read in sidechain h bonding atoms
      if(!strncmp(type, "scsc", 4))
	{
	  if(inResidue1)
	    {
      ...
      else if(!strncmp(type, "mcsc", 4))
	{
	  // A Cuff December 2005. Read in mainchain h bonding atoms
	  if(inResidue1)
	    {
	      /* storing location of where, if res 1 is a hydrogen acceptor,
		 the *partner* donor atoms like to go */
	      if(!strncmp(buffer, "mainchaindonate", 6))
		{
		  sscanf(buffer, "%s %d %d %d %d", junk, &x, &y, &z, &count);
		  gDonate[x][y][z] = count; 
		}
	      else if(!strncmp(buffer, "mainchainaccept", 6))
		{
		  sscanf(buffer, "%s %d %d %d %d", junk, &x, &y, &z, &count);
		  gAccept[x][y][z] = count; 
		}
	...
	{
	  printf("Type not mcsc or scsc but %s\n", type);
	}
      if(inResidue2)
      {
...
BOOL ParseCmdLine(int argc, char **argv, REAL *cutoff, BOOL *hbplus, char *type,
	 ...
	 case 't':
	   ...
	   break;
         ...
         case 'm':
	      ...
	      break; */
	     if((!argc) || !sscanf(argv[0], "%s", matrix_file))
	     return(FALSE);
	   break;
...
// A Cuff December 2005. Modified to include sidechain sidechain or mainchain sidechain flag

void Usage(void)
{
   fprintf(stderr, "\nCheckHBond v1.0 (c) 2002, Alison Cuff,University of Reading\n\n");
   fprintf(stderr, "USAGE: checkhbond [-c cutoff] [-t type] [-p hatom1 hatom2][-m matrix file]\n");
   ...
   fprintf(stderr, "  -t Type of hydrogen bond assessed\n");
   fprintf(stderr, "     Input mcsc for mainchain-sidechain, scsc for sidechain-sidechain bond\n");
