...
BOOL ParseCmdLine(int argc, char **argv, char *hbondtype, char *inputfile, char *outputfile);
void Usage(void);
NAMES *InitializeDomainList(FILE *fp);
char *FindStructureLocation(NAMES *names, BOOL *tempflag);
BOOL CalcAndStoreHBondData(HBOND *hb, char *hbondtype, NAMES *names, FILE *out);
...
void FindHAtoms(PDB *resA, PDB *stopA, PDB *resB, PDB *stopB, HBOND *hb, char *hbondtype);
void ClearArrays(void);
void StoreHBondingPosition(PDB *start, PDB *next, HBOND *hb, char *hbondtype);
...
int main (int argc, char *argv[])
{
   ...
   HBOND *hb;
   char inputfile[160], outputfile[160], hbondtype[3];
   NAMES *names;
   
   inputfile[0] = outputfile[0] = '\0';
   
   if(ParseCmdLine(argc, argv, hbondtype, inputfile, outputfile))
   {
      ...
      if(OpenStdFiles(inputfile, outputfile, &in, &out))
      {
         if((hb = InitializeHbondTypes()) !=NULL)
         {
            ...
            {
               if(!CalcAndStoreHBondData(hb, hbondtype, names, out))
               {
                  return(1);
               }
            }
                           ...
                           for(start=pdb; start!=NULL; start=next)
                           {
                              ...
                              if((!strncmp(start->resnam, h->residue, 3)))
                              {
                                 ...
                                 */
                                 ...
                                 {
				   StoreHBondingPosition(start, next, hb, hbondtype);
					 ...
					 /* A Cuff December 2005. Check for sidechain sidechain h bonds */
					 if(!strncmp(hbondtype, "sc", 2))
					   {
					 ...
					 /* A Cuff, December 2005. Check for mainchain sidechain h bonds */
					     	 else if(!strncmp(hbondtype, "mc", 2))
					     	   {
					    
					  if(((IsHBonded(start,nextres,HBOND_BS))
					 	    !=0) 
						    || ((IsHBonded(start,nextres,HBOND_SB))
					 	!=0))
				         {
   ...
   for(d=resA; d!=stopA; NEXT(d))
   {
      ...
      if(isDonor(d, hb, h_name1, h_name2, h_name3))
      {
         for(a=resB; a!=stopB; NEXT(a))
         {
		  ...
		  /* we need to differiente between partner mainchain and partner sidechain residues */
...
/************************************************************************/
void StoreHBondingPosition(PDB *start, PDB *stop, HBOND *hb, char *hbondtype)
{
  PDB *p;
      ...
      if(!strncmp(start->resnam, h->residue, 3))
	{
	  for(p = start; p!=stop; NEXT(p))
	      ...
	      if(!strncmp(p->atnam, h->hatom, 3))
		{
		  ...
		  if(h->donate)
		    {
		      /* A Cuff December 2005. If sidechain sidechain, then only store sidechain sidechain donor capable atoms */
		      if(!strncmp(hbondtype, "sc", 2))
			{
			  if(h->mainchain == 0)
			    {
		      ...
		      else /* A Cuff December 2005 store them all, including mainchain atoms */
			{
			  gDonate[(int)(p->x/DIV) + OFFSET] 
				[(int)(p->y/DIV) + OFFSET]
				[(int)(p->z/DIV) + OFFSET]++;
